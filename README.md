...
DetailSearch Linker searches **other note bodies on demand** for terms from the **current note** (auto-extracted or selected), highlights anchors in the **current note**, and lets you insert heading links after previewing matches. Inserted links follow Obsidian's **Use Wikilinks** setting (wikilink or Markdown link).
...
- **Heading links** — uses Obsidian's official link generator and follows the configured internal-link format; links to note start when no heading precedes the hit.
...
Search terms cannot contain `|`, `[`, or `]`. Japanese punctuation such as `、` is allowed. Inserted links are generated by Obsidian and follow the user's internal-link format setting.
...
- **Search term link syntax** — `|`, `[`, `]` are rejected in queries.
